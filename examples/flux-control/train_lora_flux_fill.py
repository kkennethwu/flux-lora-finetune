...
# Flux-fill is not in controlnet logic, we only do LoRA on it.
...
def prepare_mask_latents(
    ...
    # Calculate the height and width of the latents
    ...
    if masked_image.shape[1] == num_channels_latents:
        ...
        masked_image_latents = encode_images(masked_image, vae, dtype)
    ...
    # Resize mask to latents shape for concatenation
...
def log_validation(flux_transformer, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image_path in zip(validation_prompts, validation_images):
        ...
        # For validation, we need both image and mask
        # Assuming validation images are pairs or masks are derived from images
        mask_path = validation_image_path.replace('.', '_mask.')
        try:
            validation_mask = load_image(mask_path)
        except:
            # Create a simple center mask if no mask file exists
            validation_mask = Image.new('L', (args.resolution, args.resolution), 255)
            # Create a rectangular mask in the center
            from PIL import ImageDraw
            draw = ImageDraw.Draw(validation_mask)
            draw.rectangle([args.resolution//4, args.resolution//4, 3*args.resolution//4, 3*args.resolution//4], fill=0)
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            for log in image_logs:
                ...
                validation_image = log["validation_image"]
                validation_mask = log["validation_mask"]
                formatted_images.append(wandb.Image(validation_image, caption="Original"))
...
def save_model_card(repo_id: str, image_logs=None, base_model=str, repo_folder=None):
    ...
    if image_logs is not None:
        ...
        for i, log in enumerate(image_logs):
            ...
            validation_image = log["validation_image"]
            validation_mask = log["validation_mask"]
            validation_image.save(os.path.join(repo_folder, "image_original.png"))
            validation_mask.save(os.path.join(repo_folder, "image_mask.png"))
            img_str += f"prompt: {validation_prompt}\n"
            images = [validation_image, validation_mask] + images
...
These are Fill LoRA weights trained on {base_model} for inpainting tasks.
{img_str}
...
def get_train_dataset(args, accelerator):
    ...
    if args.mask_image_column is None:
        mask_image_column = column_names[2]
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="Simple example of a Flux-Fill LoRA training script.")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "A set of paths to the original image to be inpainted, evaluated every `--validation_steps`"
...
def collate_fn(examples):
    ...
    return {"pixel_values": pixel_values, "mask_values": mask_values, "captions": captions}
...
def main(args):
    ...
    # Note: No need to modify input channels for Flux-Fill
    ...
    # Create a pipeline for text encoding
    text_encoding_pipeline = FluxFillPipeline.from_pretrained(
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_transformer):
                ...
                # Create masked image and encode
                mask_image = batch["mask_values"]
                masked_image = batch["pixel_values"] * (1 - mask_image)

                # Prepare mask and masked image latents for Flux-Fill
                mask, masked_image_latents = prepare_mask_latents(
                    mask_image.to(accelerator.device),
                    masked_image.to(accelerator.device),
                    batch["pixel_values"].shape[0],
                    vae.config.latent_channels,
                    1,  # num_images_per_prompt
                    batch["pixel_values"].shape[2],  # height
                    batch["pixel_values"].shape[3],  # width
                    weight_dtype,
                    accelerator.device,
                    None,  # generator
                ...
                # Predict - Key difference: concatenate on dim=2 for Flux-Fill
